...
************************************/
	...
	for( i = 0 ; i < NTOTAL_COMPLEX ; i++) {
#ifdef WITH_ROTATION
		dfldo.vx[i] += 2.0 * param.omega * fldi.vy[i]*cos(theta_lat);
		dfldo.vy[i] -= 2.0 * param.omega * (fldi.vx[i]*cos(theta_lat) - fldi.vz[i]*sin(theta_lat));
		dfldo.vx[i] -= 2.0 * param.omega * fldi.vy[i]*sin(theta_lat);
#endif
#ifdef WITH_SHEAR
...
void implicitstep(
	...
	for( i = 0 ; i < NTOTAL_COMPLEX ; i++) {
#ifndef SGS
		q0 = exp( - nu * dt* pow(k2t[i],VISCOSITY_ORDER) );
		fldi.vx[i] = fldi.vx[i] * q0;
		fldi.vy[i] = fldi.vy[i] * q0;
		fldi.vz[i] = fldi.vz[i] * q0;
#endif	// SGS
...
#ifdef BOUSSINESQ
		q0 = exp( - nu_th * dt* pow(k2t[i],VISCOSITY_ORDER) );
		fldi.th[i] = fldi.th[i] * q0;
#endif
#ifdef MHD
		q0 = exp( - eta * dt* pow(k2t[i],MHD_VISCOSITY_ORDER) );
