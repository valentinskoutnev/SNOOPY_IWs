...
double twodcorrelation_complex(const double complex q[],const double complex r[]) {
	...
	if (creal( 2.0*q[ IDX3D ] * conj( r[ IDX3D ] ) )!=0.0) MPI_Printf("E=%f,j=%d, vi=%f, fvi=%f\n", energ_tot,j,abs(q[ IDX3D ]/((double) NTOTAL)),abs(r[ IDX3D ]/((double) NTOTAL))  );
