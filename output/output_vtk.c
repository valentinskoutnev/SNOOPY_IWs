...
void output_vtk(struct Field fldi, const int n, double t) {
	...
	if(param.output_vorticity)
		num_remain_field +=3;
	if(param.subtract_base_flow_IW)
		num_remain_field +=1;
	...
	if(param.subtract_base_flow_IW){
		//MPI_Printf("Out putting vtk wo baseflow: int %d\n",param.subtract_base_flow_IW);
		for( i = 0 ; i < NTOTAL_COMPLEX ; i++) {
			if ((int) round(kz[i]) == (int) param.IW_kz && (int) round(kx[i]) == (int) param.IW_kx && (int) round(ky[i]) == (int) param.IW_ky )
				w4[i] = 0;	// set amplitude of IW to zero
			else
				w4[i] = fldi.vz[i];
		}
		
		if(rank==0) fprintf(ht, "vz_mbf 1 %d float\n",array_size);
		write_vtk(ht,w4,t);
	}
#ifndef MPI_SUPPORT
#ifdef WITH_PARTICLES
