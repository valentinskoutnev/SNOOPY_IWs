...
// Transform a few basic variables.
	...
	for( i = 0 ; i < param.timevar_vars.length ; i++ ) {
		...
		else if(!strcmp(param.timevar_vars.name[i],"l_diss")) {
			double diss_rate;
			diss_rate  = dissipationRate(fldi.vx,fldi.vy,fldi.vz);
			reduce(&output_var,1);
			output_var = pow( 1.0/param.reynolds/param.reynolds/diss_rate ,0.25);
		}
		else if(!strcmp(param.timevar_vars.name[i],"ratio_lx_to_l_diss")) {
			double diss_rate;
			diss_rate  = dissipationRate(fldi.vx,fldi.vy,fldi.vz);
			reduce(&output_var,1);
			output_var = param.lx/pow( 1.0/param.reynolds/param.reynolds/diss_rate ,0.25);
		}
		...
		else if(!strcmp(param.timevar_vars.name[i],"Svxvy")) {
			// incompressible reynolds stress
			output_var=param.shear*compute_2correlation(wr1,wr2);
			reduce(&output_var,1);
		}
		else if(!strcmp(param.timevar_vars.name[i],"forc")) {
			// incompressible reynolds stress
#ifdef FORCING_TCORR
			output_var=twodcorrelation_complex(fldi.vx,fvx)+twodcorrelation_complex(fldi.vy,fvy)+twodcorrelation_complex(fldi.vz,fvz);
			reduce(&output_var,1);
#else
			output_var=0.0;
			reduce(&output_var,1);
