...
// Transform a few basic variables.
	...
	for( i = 0 ; i < param.timevar_vars.length ; i++ ) {
		...
		if(!strcmp(param.timevar_vars.name[i],"t")) {
			...
			// incompressible reynolds stress
			double sumCx,sumCy,sumCz;
			output_var=twodcorrelation_complex(fldi.vx, fvx)+twodcorrelation_complex(fldi.vy, fvy)+twodcorrelation_complex(fldi.vz, fvz);
//			MPI_Printf("vx*fvx=%f\n",twodcorrelation_complex(fldi.vx, fvx));
//			MPI_Printf("vy*fvy=%f\n",twodcorrelation_complex(fldi.vy, fvy));
//			MPI_Printf("vz*fvz=%f\n",twodcorrelation_complex(fldi.vz, fvz));
			MPI_Printf("sum=%f\n",twodcorrelation_complex(fldi.vx, fvx)+twodcorrelation_complex(fldi.vy, fvy)+twodcorrelation_complex(fldi.vz, fvz));
			sumCx=twodcorrelation_complex(fldi.vx, fvx);
			MPI_Printf("sumCx=%f\n",sumCx);

			sumCy=twodcorrelation_complex(fldi.vy, fvy);
			MPI_Printf("sumCy=%f\n",sumCy);

			sumCz=twodcorrelation_complex(fldi.vz, fvz);
			MPI_Printf("sumCz=%f\n",sumCz);
			reduce(&output_var,1);
			MPI_Printf("output_var=%f\n",output_var);
