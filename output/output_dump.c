...
#ifdef FORCING_TCORR
	// Do we have tide field in the dump?
	if(included_field & 16) {
		// Yes
		MPI_Printf("Reading current forcing fields\n");
		read_field(ht, fvx);
		read_field(ht, fvy);
		read_field(ht, fvz);
	}
		...
		//No
		ERROR_HANDLER( ERROR_WARNING, "No forcing fields in the dump, using initial conditions.");
	}
#endif
